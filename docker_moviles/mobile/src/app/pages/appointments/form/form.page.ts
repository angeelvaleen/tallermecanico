...
export class FormPage implements OnInit {
  ...
  async loadAppointment(): Promise<void> {
    ...
    try {
      const response = await axios.get<{ data: AppointmentDetail }>(
        `${environment.apiUrl}/appointments/${encodeURIComponent(this.id)}`,
      );
  ...
  async saveAppointment(): Promise<void> {
    ...
    try {
      ...
      const alert = await this.alertController.create({
        header: this.isEdition ? "Cita actualizada" : "Cita guardada",
